...
module.exports = {
  addUser: async (req, res, next) => {
    let { firstName, lastName, userName, email, password,parents } = req.body;

    // Simple validation
    if (!firstName || !lastName || !userName ||  !email || !password || !parents) {
  ...
  getAllUser: async(req, res, next) => {
    try {
      ...
      let users = await User.find({role:"emp"});
      if(users.length==0){
  ...
  connectSlack: async (req, res, next) => {
    try {
      ...
      const isUser = await User.findById(state);
      if (!isUser) {
